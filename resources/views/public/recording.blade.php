    ...
    <section class="jelajah-container py-8 sm:py-12">
        ...
        <div class="mt-5 grid gap-8 lg:grid-cols-[minmax(0,1.6fr)_minmax(0,1fr)]">
            <div>
                ...
                @elseif ($recording->downloadUrl())
                    <div class="rounded-card-lg border border-hairline bg-surface p-8 text-center">
                        ...
                        <p class="mt-4 font-medium text-navy-900">{{ $recording->type->label() }}</p>
                ...
                <h1 class="mt-6 font-display text-2xl text-navy-900 sm:text-3xl text-pretty">
                    {{ $recording->title }}
                </h1>
            ...
            <aside class="space-y-5">
                <x-ui.card>
                    <h2 class="font-semibold text-navy-900">Program Ini</h2>
                    <p class="mt-2 text-sm text-ink-soft text-pretty">{{ $event->title }}</p>
                    <dl class="mt-4 space-y-2.5 text-sm">
                        <div class="flex justify-between gap-3">
                            <dt class="text-ink-muted">Tarikh</dt>
                            <dd class="text-right text-navy-900">{{ $event->dateLabel() }}</dd>
                        </div>
                        @if ($event->speaker)
                            <div class="flex justify-between gap-3">
                                <dt class="text-ink-muted">Penceramah</dt>
                                <dd class="text-right text-navy-900">{{ $event->speaker->name }}</dd>
                            </div>
                        @endif
                    </dl>
                ...
                @if ($others->isNotEmpty())
                    <x-ui.card>
                        <h2 class="font-semibold text-navy-900">Rakaman Lain</h2>
