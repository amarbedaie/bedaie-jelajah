<x-layouts.admin title="Kehadiran" heading="Kehadiran">
        ...
        <section class="mb-8">
            ...
            <ul class="grid gap-4 sm:grid-cols-2">
                @foreach ($today as $event)
                    <li class="rounded-card border border-brand-600/30 bg-brand-50 p-5">
    ...
    <section class="mb-8">
        <h2 class="mb-3 text-lg font-semibold text-ink">Akan Datang</h2>
        @if ($upcoming->isEmpty())
            ...
            <x-jelajah.admin-table :headers="['Program', 'Tarikh', 'Lokasi', 'Berdaftar', '']">
                @foreach ($upcoming as $event)
                    <tr class="hover:bg-mist/40">
                        ...
                        <td class="px-4 py-3 text-ink">{{ number_format($event->seatsTaken()) }}</td>
    ...
    <section>
        <h2 class="mb-3 text-lg font-semibold text-ink">Program Selesai</h2>
        @if ($recent->isEmpty())
