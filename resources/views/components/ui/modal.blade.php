...
<div x-data="{ open: false }"
    ...
    <div x-show="open" class="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
        <div x-show="open" x-transition.opacity class="absolute inset-0 bg-char-900/50 backdrop-blur-sm"
            ...
            @if ($title)
                <div class="sticky top-0 flex items-center justify-between gap-4 border-b border-hairline bg-surface px-5 py-4 sm:px-6">
                    ...
                    <button type="button" x-on:click="open = false"
                            class="tap-target -mr-2 flex items-center justify-center rounded-full text-ink-muted hover:bg-mist hover:text-ink"
                            aria-label="Tutup">
