...
<div {{ $attributes->merge([
    ...
    <div class="flex items-center justify-center rounded-2xl bg-clay-50 {{ $compact ? 'w-11 h-11' : 'w-14 h-14' }}">
        <x-ui.icon :name="$icon" class="{{ $compact ? 'w-5 h-5' : 'w-7 h-7' }} text-clay-400" />
    </div>
    <h2 class="mt-4 {{ $compact ? 'text-base' : 'text-lg' }} font-semibold text-ink">{{ $title }}</h2>
    @if ($description)
