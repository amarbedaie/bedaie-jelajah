...
<body class="min-h-screen bg-cream antialiased">
            ...
            <div class="flex h-16 items-center justify-between gap-4">
                ...
                <div class="flex items-center gap-1.5">
                    <a href="{{ route($nav.'.notifikasi') }}"
                       class="tap-target relative grid place-items-center rounded-xl text-ink-soft transition hover:bg-mist hover:text-ink"
                            ...
                            <div class="p-1.5">
                                @if ($user->isAdmin())
                                    <a href="{{ route('admin.dashboard') }}"
                                       class="tap-target flex items-center gap-2.5 rounded-xl px-3 text-sm text-ink hover:bg-mist">
    ...
    <main id="kandungan" class="jelajah-container py-7 sm:py-9 {{ $wide ? '!max-w-[90rem]' : '' }}">
        @if ($heading)
            <div class="mb-6">
                <h1 class="text-2xl font-semibold tracking-tight text-ink sm:text-3xl">{{ $heading }}</h1>
                @if ($subheading)
