...
class PosterGenerator
{
    ...
    private function compose(Event $event): \Imagick
    {
        $w = self::W;
        $h = self::H;

        $canvas = $this->background($w, $h, 0.62);
        ...
        $draw->setFillColor('rgba(255,255,255,0.10)');
        $draw->setStrokeColor('rgba(255,255,255,0.18)');
        $draw->setStrokeWidth(1);
        $draw->roundRectangle(80, 232, 80 + $pillW, 286, 27, 27);
        $draw->setStrokeWidth(0);
        $draw->setStrokeColor('transparent');

        $this->text($draw, $category, 108, 266, 21, self::BRAND_SOFT, 'sans-bold', 3.5);

        // ── Slogan ──
        $this->text($draw, 'MEMBAWA ILMU, MENGHIDUPKAN UMMAH', 80, 356, 24, self::MUTED, 'sans', 2);

        // ── Tajuk ──
        $y = 452;
        foreach ($this->wrap($event->title, 22, 3) as $line) {
            $this->text($draw, $line, 80, $y, 66, '#FFFFFF', 'serif-bold');
            $y += 84;
        }
        ...
        if ($event->theme) {
            ...
            foreach ($this->wrap($event->theme, 36, 2) as $line) {
                $this->text($draw, $line, 80, $y, 34, self::BRAND_SOFT, 'sans');
                $y += 46;
            }
        }
        ...
        $rows = array_values(array_filter([
            ...
            $event->speaker ? ['PENCERAMAH', $event->speaker->name, true] : null,
        ...
        $rowHeight = (int) max(58, min(74, $available / max(1, count($rows))));

        foreach ($rows as [$label, $value]) {
            $this->text($draw, $label, 80, $y, 20, self::MUTED, 'sans', 3);
            $this->text($draw, Str::limit($value, 36), 80, $y + 36, 32, '#FFFFFF', 'sans-bold');
            $y += $rowHeight;
        }

        // ── Kaki ──

        $draw->setFillColor('rgba(255,255,255,0.16)');
        ...
        $this->text($draw, 'DAFTAR SEKARANG', 336, $footerY + 108, 21, self::MUTED, 'sans', 3);
        $this->text($draw, $event->shortUrl(), 336, $footerY + 156, 32, '#FFFFFF', 'sans-bold');
        $this->text($draw, 'Tempat terhad · Anjuran BeDaie', 336, $footerY + 202, 21, self::MUTED, 'sans');
    ...
    private function composeHero(Event $event): \Imagick
    {
        $w = self::W;
        $h = 608;

        $canvas = $this->background($w, $h, 0.5);
        ...
        foreach ($lines as $line) {
            $this->text($draw, $line, 72, $y, 58, '#FFFFFF', 'serif-bold');
            $y += 76;
        }

        if ($event->theme) {
            $this->text($draw, Str::limit($event->theme, 46), 72, $y + 18, 30, self::BRAND_SOFT, 'sans');
        }
     ...
     */
    private function background(int $w, int $h, float $rosetteScale): \Imagick
    {
        $canvas = new \Imagick;
        $canvas->newPseudoImage($w, $h, 'gradient:'.self::NAVY.'-'.self::NAVY_DEEP);

        // Cahaya sebesar kanvas, dipusatkan ke kanan atas melalui saiz, bukan offset.
        ...
        $glow->evaluateImage(\Imagick::EVALUATE_MULTIPLY, 0.30, \Imagick::CHANNEL_ALL);
     ...
     */
    private function drawRosette(\Imagick $canvas, float $cx, float $cy, float $r): void
    {
        $draw = new \ImagickDraw;
        ...
        $ring = function (float $radius, float $opacity, float $width, int $step = 1)
            use ($draw, $cx, $cy, $n) {
            $draw->setStrokeColor("rgba(255,255,255,{$opacity})");
        ...
        $draw->setStrokeColor('rgba(255,255,255,0.07)');
    ...
    private function drawGirihField(\Imagick $canvas, ?int $width = null, ?int $height = null): void
    {
        $width ??= self::W;
        $height ??= self::H;
        ...
        $draw = new \ImagickDraw;
        $draw->setFillColor('transparent');
        $draw->setStrokeColor('rgba(255,255,255,0.055)');
    ...
    private function drawLogo(\ImagickDraw $draw): void
    {
        $draw->setFillColor(self::BRAND);
        $draw->setStrokeColor('transparent');
        ...
        $this->text($draw, 'BeDaie', 178, 124, 38, '#FFFFFF', 'sans-bold');
        $this->text($draw, 'JELAJAH', 178, 154, 18, self::BRAND_SOFT, 'sans-bold', 7);
    }
